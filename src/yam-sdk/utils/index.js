...
export const delegate = async (yam, account, onTxHash) => {
  return yam.contracts.yam.methods.delegate(account).send({from: account }, async (error, txHash) => {
    if (error) {
        onTxHash && onTxHash('')
...
export const didDelegate = async (yam, account) => {
  ...
  return await yam.contracts.yam.methods.delegates(account).call() === account
}
...
export const getProposals = async (yam) => {
  ...
  // TODO: make change when runs at mainnet at here! 'from'

  // kovan
  // (to be change)
  const v1Proposals = await yam.contracts.gov.getPastEvents("ProposalCreated", {fromBlock: 11213909})
  let proposals = [];
  // let v1Descriptions = [];
    ...
    if (proposals[i].gov === "gov") {
      ...
      if (power === 0) {
        power =  new BigNumber(await
                  yam.contracts.yam.methods.getPriorVotes(account, proposals[i].start).call()
                ).div(BASE24).toNumber();
      }
      powers.push({
...
export const getCurrentVotingPower = async (yam, account) => {
  ...
  return new BigNumber(await yam.contracts.yam.methods.getCurrentVotes(account).call()).dividedBy(BASE24).toNumber()
}
...
export const getScalingFactor = async (yam) => { return new BigNumber(await
  yam.contracts.yam.methods.yamsScalingFactor().call())
}
...
const yamToFragment = async (yam, amount) => {
  let BASE24 = new BigNumber(10).pow(24);
  let scalingFactor = new BigNumber(await yam.contracts.yam.methods.yamsScalingFactor().call());
...
export const claimVested = async (yam, account, onTxHash) => {
  return await yam.contracts.migrator.methods.claimVested().send({from: account});
}
...
export const scalingFactors = async (yam) => {
  ...
  let rebases = await yam.contracts.yam.getPastEvents('Rebase', {fromBlock: 10886913, toBlock: 'latest'});
  let scalingFactors = [];
  let blockNumbers = [];
