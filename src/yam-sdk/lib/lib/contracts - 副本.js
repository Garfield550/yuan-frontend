...
export class Contracts {
  constructor(
    ...
    this.mkr_pool = new this.web3.eth.Contract(MKRPoolJson.abi);
    this.df_pool = new this.web3.eth.Contract(DFPoolJson.abi);
    this.dusdt_pool = new this.web3.eth.Contract(DUSDTPoolJson.abi);
    // 添加新pool
    ...
    this.HT_ETH_pool = new this.web3.eth.Contract(DUSDTPoolJson.abi);
    this.COMP_ETH_pool = new this.web3.eth.Contract(DUSDTPoolJson.abi);
    ...
    this.df = new this.web3.eth.Contract(ERC20Json.abi);
    this.dusdt = new this.web3.eth.Contract(ERC20Json.abi);
  ...
  setProvider(
    ...
    const contracts = [
      ...
      { contract: this.dusdt_pool, json: DUSDTPoolJson },
      // 添加新contract
      ...
      { contract: this.HT_ETH_pool, json: addressMapJSON.HT_ETH },
      { contract: this.COMP_ETH_pool, json: addressMapJSON.COMP_ETH },
    ]
    ...
    this.uni_router.options.address = addressMap["UNIRouter"];
    this.mkr.options.address = addressMap["MKR"];
    this.df.options.address = addressMap["DF"];
    this.dusdt.options.address = addressMap["dUSDT"];
    this.pools = [
      { "tokenAddr": this.mkr.options.address, "poolAddr": this.mkr_pool.options.address },
      // { "tokenAddr": this.mkr.options.address, "poolAddr": "0xF00488175d52666c4AA0897c06708Eb0b7ff367e" },
      { "tokenAddr": this.df.options.address, "poolAddr": this.df_pool.options.address },
      { "tokenAddr": this.dusdt.options.address, "poolAddr": this.dusdt_pool.options.address },
      ...
      { "tokenAddr": this.HT_ETH_pool.options.address, "poolAddr": this.HT_ETH_pool.options.address },
      { "tokenAddr": this.COMP_ETH_pool.options.address, "poolAddr": this.COMP_ETH_pool.options.address },
    ]
